...
jQuery.fn.rater	= function(options) {
	...
	//显示点击提示
	var clickTips= jQuery('<div class="rater-click-tips"><span>Click stars to rate!</span></div>');
	...
	content.find('.rater-star-item').mouseover(function() {
		...
		var current_value	= settings.min + (current_number - 1) * settings.step;
                ...
                //window.location.href="operate?optype=7&eid="+eid+"&rate="+current_value;
		
		//显示当前分值
		if (typeof settings.title_format == 'function') {
			jQuery(this).parents().nextAll('.rater-star-result').html(current_value+'&nbsppoints&nbsp;'+settings.title_format(current_value));
...
$(function(){
	...
	info_format	: function(value) {
		...
		switch (value) {
			case 1 : 
				info	= '<div class="info-box">Only one point!</div>';
				break;
			case 2 : 
				info	= '<div class="info-box">Two points!</div>';
				break;
			case 3 : 
				info	= '<div class="info-box">Three points!</div>';
				break;
			case 4 : 
				info	= '<div class="info-box">Four points!</div>';
				break;
			case 5 : 
				info	= '<div class="info-box">Five points!Awesome!</div>';
				break;
			default :
				info = value;
				break;
